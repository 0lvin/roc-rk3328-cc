...
static int rk808_regulator_probe(struct platform_device *pdev)
{
	...
	ret = rk808_regulator_dt_parse_pdata(&pdev->dev, &client->dev,
					     rk805_reg_matches, RK805_NUM_REGULATORS,
					     rk808->regmap, pdata);
	if (ret < 0)
		return ret;
	...
	for (i = 0; i < nregulators; i++) {
		dev_err(&client->dev, "%s:%d: [%d] = %s (%s)\n", __FILE__, __LINE__, i, config.of_node ? config.of_node->name : "<unknown>", client->dev.of_node->name);

		if (!rk805_reg_matches[i].init_data ||
		    !rk805_reg_matches[i].of_node) {
			dev_err(&client->dev, "%s:%d: Nod device [%d]", __FILE__, __LINE__, i);
			continue;
		}
